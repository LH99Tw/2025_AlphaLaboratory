...
### 🌟 포인트 컬러 (따뜻한 금색 계열)
- **Accent Primary**: `#D4AF37` - 부드러운 골드 (메인 액센트)
- **Accent Gold**: `#B8860B` - 다크 골든로드
- **Accent Gradient**: `linear-gradient(135deg, #D4AF37 0%, #B8860B 100%)`
- **Liquid Gold**: `rgba(212, 175, 55, 0.12)` - 부드러운 반투명 금색
- **Liquid Gold Border**: `rgba(184, 134, 11, 0.3)` - 금색 테두리
- **Liquid Gold Hover**: `rgba(212, 175, 55, 0.2)` - 금색 호버
- **Liquid Gold Gradient**: `linear-gradient(135deg, rgba(212, 175, 55, 0.15) 0%, rgba(184, 134, 11, 0.08) 50%, rgba(160, 120, 8, 0.05) 100%)` - 3단계 부드러운 그라데이션
- **Glowing Text**: `linear-gradient(135deg, #D4AF37 0%, #B8860B 50%, #A07808 100%)` - 빛나는 텍스트 효과
- **Glowing Text Hover**: `linear-gradient(135deg, #F4E4BC 0%, #D4AF37 50%, #B8860B 100%)` - 호버 시 더 밝은 빛나는 효과
...
### 🪪 브랜드 아이콘 & 내비게이션 규칙
- **로고**: 흰색 원형 아이콘(문자 `○`)과 `LAB` 워드마크 조합을 기본으로 사용.
- **프로필 이모지**: 사용자 정보 버튼에는 유리 재질의 원형 배경과 사용자 이모지를 표준으로 적용.
- **사이드바 토글 상태 간격**: 축소 모드에서는 항목 간 수직 간격을 4px 이하로 유지해 아이콘이 밀집되도록 하고, 좌우 여백은 8px 안쪽으로 제한.
